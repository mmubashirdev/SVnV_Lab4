...
While moving, the robot continuously checks its surroundings. If it detects an obstacle, it temporarily stops its normal navigation and enters an obstacle-avoidance mode. Once the obstacle has been successfully avoided, the robot continues navigating toward the destination.

When the robot reaches the destination, it starts the delivery process. After the package has been successfully delivered, the robot begins its journey back to the warehouse.
